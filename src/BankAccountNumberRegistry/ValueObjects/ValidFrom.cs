namespace BankAccountNumberRegistry
{
    ...
    public struct ValidFrom : IEquatable<ValidFrom>, IComparable<ValidFrom>
    {
        ...
        public ValidFrom(DateTime? dateTime) => DateTime = dateTime;

        public ValidFrom(int year, int month, int day) => DateTime = new DateTime(year, month, day);

        public static implicit operator DateTime? (ValidFrom validFrom) => validFrom.DateTime;

        public override string ToString() => DateTime.HasValue ? DateTime.Value.ToString("yyyy-MM-dd") : "~";
