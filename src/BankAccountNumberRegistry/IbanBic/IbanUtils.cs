...
namespace BankAccountNumberRegistry.IbanBic
{
    using System;
    using System.Text;
    ...
    public static class IbanUtils
    {
        ...
        private static void validateCheckDigit(string iban)
        {
            if (calculateMod(iban) != 1)
            {
                ...
                throw new InvalidCheckDigitException( $"{iban} heeft een ongeldige controlecijfer {checkDigit}. Verwacht controlecijfer is {expectedCheckDigit}.", expectedCheckDigit, checkDigit );
            }
        }
        ...
        private static void validateEmpty (string iban)
        {
            if (string.IsNullOrEmpty(iban))
            {
                throw new IbanFormatException( "Een lege invoer kan geen geldig IBAN zijn.", IbanFormatViolation.IBAN_NOT_EMPTY_OR_NULL );
            }
        }
        ...
        private static void validateCountryCode (string iban)
        {
            ...
            {
                throw new IbanFormatException( "Invoer moet 2 letters bevatten voor de landcode.", IbanFormatViolation.COUNTRY_CODE_TWO_LETTERS, iban );
            }
            ...
            {
                throw new IbanFormatException( "IBAN land code moet bestaan uit hoofdletters.", IbanFormatViolation.COUNTRY_CODE_UPPER_CASE_LETTERS, iban );
            }
            ...
            {
                throw new IbanFormatException( "IBAN bevat een landcode die niet bestaat.", IbanFormatViolation.COUNTRY_CODE_EXISTS, iban );
            }

            BBanStructure structure = Bban.GetStructureForCountry( countryEntry );
            if (structure == null)
            {
                throw new UnsupportedCountryException( "IBAN bevat een landcode die niet ondersteund wordt.", countryCode );
            }
        }
        ...
        private static void validateCheckDigitPresence(string iban)
        {
            ...
            {
                throw new IbanFormatException( "IBAN moet twee cijfers bevatten voor het controlecijfer.", IbanFormatViolation.CHECK_DIGIT_TWO_DIGITS, iban.Substring( _COUNTRY_CODE_LENGTH ) );
            }

            string checkDigit = GetCheckDigit( iban );
            if (!char.IsDigit(checkDigit[0]) || !char.IsDigit(checkDigit[1]))
            {
                throw new IbanFormatException( "IBAN's controlecijfer mag enkel cijfers bevatten.", IbanFormatViolation.CHECK_DIGIT_ONLY_DIGITS, checkDigit );
            }
        }
        ...
        private static void validateBbanLength(string iban, BBanStructure structure)
        {
            ...
            {
                throw new IbanFormatException( $"BBAN '{bban}' lengte is {bbanLength}, verwachte lengte is {expectedBbanLength}.",
            ...
            switch (entry.CharacterType)
            {
                case BBanEntryCharacterType.A:
                    foreach (char c in entryValue.ToCharArray())
                    {
                        if (!char.IsUpper(c))
                        {
                            throw new IbanFormatException( $"'{entryValue}' mag enkel hoofdletters bevatten.",
                ...
                case BBanEntryCharacterType.C:
                    foreach (char c in entryValue.ToCharArray())
                    {
                        ...
                        {
                            throw new IbanFormatException( $"'{entryValue}' mag enkel letters of cijfers bevatten.",
                ...
                case BBanEntryCharacterType.N:
                    foreach (char c in entryValue.ToCharArray())
                    {
                        if (!char.IsDigit(c))
                        {
                            throw new IbanFormatException( $"'{entryValue}' mag enkel cijfers bevatten.",
        ...
        private static int calculateMod (string iban)
        {
            ...
            for (int i = 0; i < reformattedIban.Length; i++)
            {
                ...
                {
                    throw new IbanFormatException( $"Ongeldig teken op plaats {i} = {numericValue}.", IbanFormatViolation.IBAN_VALID_CHARACTERS, reformattedIban[i] );
                }
        ...
        private static string changeBbanEntry(string iban, string newValue, BBanEntryType entryType)
        {
            ...
            foreach (BBanEntry entry in structure.Entries)
            {
                if (entry.EntryType == entryType)
                {
                    ...
                    {
                        throw new IbanFormatException( $"Nieuwe waarde voor {Enum.GetName( typeof( BBanEntryType ), entry.EntryType )} is te lang.", IbanFormatViolation.BBAN_ENTRY_TOO_LONG );
                    }
