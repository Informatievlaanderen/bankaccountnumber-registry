...
namespace BankAccountNumberRegistry.IbanBic
{
    ...
    public static class BicUtils
    {
        ...
        private static void validateEmpty (string bic)
        {
            if (string.IsNullOrEmpty(bic))
            {
                throw new BicFormatException( "Een lege invoer kan geen geldig BIC zijn.", BicFormatViolation.BIC_NOT_EMPTY_OR_NULL );
            }
        }
        ...
        private static void validateLength (string bic)
        {
            ...
            {
                throw new BicFormatException( $"BIC lengte moet {_BIC8_LENGTH} of {_BIC11_LENGTH} zijn.", BicFormatViolation.BIC_LENGTH_8_OR_11 );
            }
        }
        ...
        private static void validateCase (string bic)
        {
            ...
            {
                throw new BicFormatException( "BIC mag enkel hoofdletters bevatten.", BicFormatViolation.BIC_ONLY_UPPER_CASE_LETTERS );
            }
        }
        ...
        private static void validateBankCode (string bic)
        {
            ...
            foreach (char c in bankCode.ToCharArray())
            {
                if (!char.IsLetter(c))
                {
                    throw new BicFormatException( "Bankcode mag enkel letters bevatten.", BicFormatViolation.BANK_CODE_ONLY_LETTERS );
                }
            }
        ...
        private static void validateCountryCode (string bic)
        {
            ...
            {
                throw new BicFormatException( "BIC landcode moet hoofdletters bevatten.", BicFormatViolation.COUNTRY_CODE_ONLY_UPPER_CASE_LETTERS );
            }

            if (CountryCode.GetCountryCode(countryCode) == null)
            {
                throw new UnsupportedCountryException( "De gebruikte landcode wordt niet ondersteund.", countryCode );
            }
        }
        ...
        private static void validateLocationCode (string bic)
        {
            ...
            foreach (char c in locationCode.ToCharArray())
            {
                ...
                {
                    throw new BicFormatException( "Locatiecode mag enkel letters of cijfers bevatten.", BicFormatViolation.LOCATION_CODE_ONLY_LETTERS_OR_DIGITS );
                }
            }
        }
        ...
        private static void validateBranchCode (string bic)
        {
            ...
            foreach (char c in branchCode.ToCharArray())
            {
                if (!char.IsLetterOrDigit(c))
                {
                    throw new BicFormatException( "Afdelingscode mag enkel letters of cijfers bevatten", BicFormatViolation.BRANCH_CODE_ONLY_LETTERS_OR_DIGITS );
                }
            }
        }
